...
void binifstream::open(const char *filename, const Mode mode) {
   f = fopen(filename, "rb");
    if (f== NULL) {//niuzb
        char filenamess[50]={0};
        //printf("try upcase file name %s\n", lpszFileName);
        //filename = upperString((char *)lpszFileName);
        for (int i = 0; i < strlen(filename); i++) {
            filenamess[i] = toupper(filename[i]);
            //  printf("%c", filename[i]);
        }
        // printf("translate to %s\n", filename);
        f = fopen( filenamess, "rb");
        //        free(filename);
    }
   if (f == NULL)
      switch (errno) {
