...
--*/
{
   ...
   MENUITEM      rgMainMenuItem[2] = {
      ...
      {  1,      MAINMENU_LABEL_LOADGAME,  TRUE,     PAL_XY(125, 140) }
   };
...
VOID PAL_SetPlayerMaxHP(VOID)
    {
        ...
        while (iCurrent >= 0 && iCurrent <= gpGlobals->wMaxPartyMemberIndex)
        {
            ...
            gpGlobals->g.PlayerRoles.rgwHP[iPlayerRole] =gpGlobals->g.PlayerRoles.rgwMaxHP[iPlayerRole];
            gpGlobals->g.PlayerRoles.rgwMP[iPlayerRole] =gpGlobals->g.PlayerRoles.rgwMaxMP[iPlayerRole];
            iCurrent++;
        }
    }
...
--*/
{
   ...
   while (iCurrent >= 0 && iCurrent <= gpGlobals->wMaxPartyMemberIndex)
   {
      ...
      while (TRUE)
      {
         ...
         else if (g_InputState.dwKeyPress & (kKeyLeft | kKeyUp|kKeyPgUp))
         {
            iCurrent--;
            break;
         }
         else if (g_InputState.dwKeyPress & (kKeyRight | kKeyDown | kKeySearch|kKeyPgDn))
         {
            iCurrent++;
            break;
         }
