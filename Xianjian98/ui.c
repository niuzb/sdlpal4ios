...
int the_singline_box_len = 0;//used this for check mouse click coordinate. 
LPBOX
...
--*/
{
   ...
   PAL_RLEBlitToSurface(lpBitmapRight, gpScreen, PAL_XY(rect.x, rect.y));
    the_singline_box_len =rect.w; //add by niuzb
   return lpBox;
}
...
--*/
{
   ...
   while (TRUE)
   {
      ...
      else if (g_InputState.dwKeyPress & kKeySearch)
      {
       ...
       if (g_InputState.mouse_dir == kDirSouth) {//mouse up
           int x = g_InputState.x*320*1.0f/g_wInitialWidth;
           int y  = g_InputState.y*200*1.0f/g_wInitialHeight;
           int width;
                     //printf("translated %d %d \n", x, y);
           for (int i = 0; i < nMenuItem; i++) {
               DWORD pos = rgMenuItem[i].pos;
               int xx = PAL_X(pos);
               int yy = PAL_Y(pos);
               if (the_singline_box_len) {
                   width = the_singline_box_len;
               }else {
                   width = PAL_TextLen(PAL_GetWord(rgMenuItem[i].wNumWord),
                                       rgMenuItem[i].pos);
               }
